...
public class KafkaConsumer {
     ...
     * 监听预约创建消息
     */
    @KafkaListener(topics = "${notification.kafka.topics.email}")
    public void listenEmailRequested(String message) {
        log.info("收到邮件发送消息: {}", message);
        messageHandlerService.handleSendEmail(message);
    }
